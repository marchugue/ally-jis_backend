...
const PROFILE_COLUMNS =
  'id, email, full_name, username, avatar_url, bio, department, course, year_level, interests, organizations, created_at, zodiac_sign, personality_type, music_taste, movie_interests, age_range, match_gender_preference';
...
export async function updateById(id: string, payload: UpdateProfilePayload): Promise<ProfileRow> {
  ...
  if (payload.zodiac_sign !== undefined) updates.zodiac_sign = payload.zodiac_sign;
  if (payload.personality_type !== undefined) updates.personality_type = payload.personality_type;
  if (payload.music_taste !== undefined) updates.music_taste = payload.music_taste;
  if (payload.movie_interests !== undefined) updates.movie_interests = payload.movie_interests;
  if (payload.age_range !== undefined) updates.age_range = payload.age_range;
  if (payload.match_gender_preference !== undefined) updates.match_gender_preference = payload.match_gender_preference;
